...
ssh ${SSH_OPTIONS} ${STAGING_SERVER} <<EOF
    ...
    git clone --branch new-dep git@gitlab.bbdev.team:vh/subtitles-frontend.git src
    ...
    sed -i 's|SUBTITLES_FRONTEND_VERSION=.*|SUBTITLES_FRONTEND_VERSION=316|g' .env
    sed -i 's|SUBTITLES_BACKEND_URL=.*|SUBTITLES_BACKEND_URL=https://subtitles.bbdev3.kbb1.com/backend/api/v1/|g' .env
    sed -i 's|REACT_APP_API_BASE_URL=.*|REACT_APP_API_BASE_URL=https://subtitles.bbdev3.kbb1.com/backend/api/v1/|g' .env
    sed -i 's|REACT_APP_MQTT_URL=.*|REACT_APP_MQTT_URL=broker.emqx.io|g' .env
    sed -i 's|REACT_APP_MQTT_PROTOCOL=.*|REACT_APP_MQTT_PROTOCOL=wss|g' .env
    sed -i 's|REACT_APP_MQTT_PATH=.*|REACT_APP_MQTT_PATH=mqtt|g' .env
    sed -i 's|REACT_APP_MQTT_PORT=.*|REACT_APP_MQTT_PORT=8084|g' .env
    sed -i 's|REACT_APP_KEYCLOAK_URL=.*|REACT_APP_KEYCLOAK_URL=https://auth.2serv.eu/auth|g' .env
    sed -i 's|REACT_APP_KEYCLOAK_REALM=.*|REACT_APP_KEYCLOAK_REALM=master|g' .env
    sed -i 's|REACT_APP_KEYCLOAK_CLIENT_ID=.*|REACT_APP_KEYCLOAK_CLIENT_ID=kolman-dev|g' .env
    sed -i 's|REACT_APP_PORT=.*|REACT_APP_PORT=80|g' .env
